float2 ParallaxMapping(Texture2D depth_tex, SamplerState depth_sampler, float2 texCoords, float3 view_dir, float3x3 TBN, float height_scale)
{
	...
	float numLayers = lerp(maxLayers, minLayers, abs(dot(float3(0.0f, 0.0f, 1.0f), view_dir)));
	...
	float layer_depth_current = 0.0f;
	
	// calculate the step size (size of each layer)
	float step_size = 1.0f / numLayers;
	...
	float2 finalTexCoods = prevTexCoords * weight + currentTexCoords * (1.0f - weight);
