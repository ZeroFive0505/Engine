...
namespace PlayGround
{
    ...
    class Renderer : public SubModule
    {
    public:
        ...
        #define DEBUG_COLOR Math::Vector4(0.0f, 1.0f, 0.0f, 1.0f)
        ...
        inline const RHI_Viewport& GetViewport() const { return m_viewport; }
        void SetViewport(float width, float height);


        inline const Math::Vector2& GetResolutionRender() const { return m_resolution_render; }
        void SetResolutionRender(uint32_t width, uint32_t height, bool recreate_resources = true);

        inline const Math::Vector2& GetResolutionOutput() const { return m_resolution_output; }
        void SetResolutionOutput(uint32_t width, uint32_t height, bool recreate_resources = true);

        inline std::shared_ptr<RHI_Texture> GetRenderTarget(const RenderTarget rt_enum) { return m_render_targets[static_cast<uint8_t>(rt_enum)]; }
        inline const auto& GetRenderTargets() { return m_render_targets; }

        inline float GetClearDepth() { return GetOption(Renderer::Option::ReverseZ) ? m_viewport.depth_min : m_viewport.depth_max; }

        const std::shared_ptr<RHI_Texture> GetEnvironmentTexture();
        void SetEnvironmentTexture(std::shared_ptr<RHI_Texture> texture);

        inline uint64_t GetOptions()                        const { return m_options; }
        inline void SetOptions(const uint64_t options) { m_options = options; }
        inline bool GetOption(const Renderer::Option option) const { return m_options & option; }
        ...
        inline RHI_SwapChain* GetSwapChain() const { return m_swap_chain.get(); }
        void Present();

        void Flush();

        inline RHI_Texture* GetDefaultTextureWhite()       const { return m_tex_default_white.get(); }
        inline RHI_Texture* GetDefaultTextureBlack()       const { return m_tex_default_black.get(); }
        inline RHI_Texture* GetDefaultTextureTransparent() const { return m_tex_default_transparent.get(); }
        ...
        inline Cb_Uber& GetUberBufferCpu() { return m_cb_uber_cpu; }
        ...
        inline const std::shared_ptr<RHI_Device>& GetRhiDevice() const { return m_rhi_device; }
        inline RHI_Texture* GetFrameTexture() { return GetRenderTarget(Renderer::RenderTarget::Frame_Output).get(); }
        inline uint64_t GetFrameNum()                                const { return m_frame_num; }
        inline std::shared_ptr<Camera> GetCamera()               const { return m_camera; }
        inline auto GetShaders()                                 const { return m_shaders; }
        inline RHI_CommandList* GetCmdList()                     const { return m_cmd_current; }
    ...
    private:
        ...
        const float m_depth_bias = 0.004f;
