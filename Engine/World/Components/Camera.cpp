...
namespace PlayGround
{
    ...
    {
        float near_plane_limited = Util::Max(near_plane, 0.01f);
    ...
    {
        return Util::RadiansToDegrees(m_fov_horizontal_rad);
    }
    ...
    {
        m_fov_horizontal_rad = Util::DegreesToRadians(fov);
        m_is_dirty = true;
    }
    ...
    bool Camera::Pick(shared_ptr<Entity>& picked)
    {
        ...
        vector<RayHit> hits;
        {
            const auto& entities = m_Context->GetSubModule<World>()->EntityGetAll();
            for (const auto& entity : entities)
            {
                ...
                if (distance == Util::INFINITY_)
                    continue;

                hits.emplace_back(
                    entity,                                             
                    m_ray.GetStart() + m_ray.GetDirection() * distance, 
                    distance,                                         
                    distance == 0.0f                                    
                );
            }
    ...
    {
        {
            if (m_input->GetKeyDown(EKeyCode::Click_Right) && m_input->GetMouseIsInViewport())
            {
                m_fps_control_assumed = true;
            }

            m_fps_control_assumed = m_input->GetKey(EKeyCode::Click_Right) && m_fps_control_assumed;
        }
            ...
            {
                {
                    ...
                    {
                        mouse_position.x = static_cast<float>(PlayGround::Display::GetWidth() - edge_padding - 1);
                        m_input->SetMousePosition(mouse_position);
                    }
                }
                ...
                m_mouse_smoothed = Util::Lerp(m_mouse_smoothed, mouse_delta, Util::Saturate(1.0f - m_mouse_smoothing));

                m_mouse_rotation += m_mouse_smoothed;

                m_mouse_rotation.y = Util::Clamp(-80.0f, 80.0f, m_mouse_rotation.y);

                const Quaternion xQuaternion = Quaternion::FromAngleAxis(m_mouse_rotation.x * Util::DEG_TO_RAD, Vector3::Up);
                const Quaternion yQuaternion = Quaternion::FromAngleAxis(m_mouse_rotation.y * Util::DEG_TO_RAD, Vector3::Right);
                const Quaternion rotation = xQuaternion * yQuaternion;

                m_Transform->SetLocalRotaion(rotation);
            }

            {
                m_movement_speed_max += m_input->GetMouseWheelDelta().y / 2.0f;
                m_movement_speed_max = Util::Clamp(m_movement_speed_min, numeric_limits<float>::max(), m_movement_speed_max);

                Vector3 direction = Vector3::Zero;
                if (m_input->GetKey(EKeyCode::W)) direction += m_Transform->GetForward();
                if (m_input->GetKey(EKeyCode::S)) direction += m_Transform->GetBackward();
                if (m_input->GetKey(EKeyCode::D)) direction += m_Transform->GetRight();
                if (m_input->GetKey(EKeyCode::A)) direction += m_Transform->GetLeft();
                if (m_input->GetKey(EKeyCode::Q)) direction += m_Transform->GetDown();
                if (m_input->GetKey(EKeyCode::E)) direction += m_Transform->GetUp();
                direction.Normalize();
        ...
        {
            m_movement_speed *= 1.0f - Util::Saturate(m_movement_drag * static_cast<float>(delta_time));
    ...
    {
        if (m_input->GetKeyDown(EKeyCode::F))
        {
            if (Entity* entity = m_Context->GetSubModule<World>()->GetTransformHandle()->GetSelectedEntity())
            {
