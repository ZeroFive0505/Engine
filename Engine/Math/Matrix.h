...
namespace PlayGround::Math
{
    ...
    public:
        ...
        inline Vector3 GetTranslation() const { return Vector3(m30, m31, m32); }
        ...
        static inline Quaternion RotationMatrixToQuaternion(const Matrix& mRot)
        {
            ...
            {
                sqrt = Util::Sqrt(1.0f + mRot.m00 - mRot.m11 - mRot.m22);
                half = 0.5f / sqrt;
            ...
            {
                sqrt = Util::Sqrt(1.0f + mRot.m11 - mRot.m00 - mRot.m22);
                half = 0.5f / sqrt;
            ...
            sqrt = Util::Sqrt(1.0f + mRot.m22 - mRot.m00 - mRot.m11);
            half = 0.5f / sqrt;
        ...
        {
            const int xs = (Util::Sign(m00 * m01 * m02 * m03) < 0) ? -1 : 1;
            const int ys = (Util::Sign(m10 * m11 * m12 * m13) < 0) ? -1 : 1;
            const int zs = (Util::Sign(m20 * m21 * m22 * m23) < 0) ? -1 : 1;

            return Vector3(
                static_cast<float>(xs) * Util::Sqrt(m00 * m00 + m01 * m01 + m02 * m02),
                static_cast<float>(ys) * Util::Sqrt(m10 * m10 + m11 * m11 + m12 * m12),
                static_cast<float>(zs) * Util::Sqrt(m20 * m20 + m21 * m21 + m22 * m22)
            );
        }
        ...
        {
            const float yScale = Util::Cot(fieldOfView / 2);
        ...
        inline Matrix Transposed() const { return Transpose(*this); }
        ...
        inline Matrix Inverted() const { return Invert(*this); }
        ...
        inline void Decompose(Vector3& scale, Quaternion& rotation, Vector3& translation) const
        {
        ...
        bool Equals(const Matrix& rhs)
        {
            ...
            for (unsigned i = 0; i < 16; ++i)
            {
                if (!Util::Equals(data_left[i], data_right[i]))
                    return false;
            }
        ...
        inline const float* Data() const { return &m00; }
